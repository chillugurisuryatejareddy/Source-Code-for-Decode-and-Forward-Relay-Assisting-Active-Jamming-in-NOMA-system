% Without Jamming.
function [OP1,OP2,OP1E,OP2E,cap_1,cap_2,cap_1E,cap_2E,cap_S1,cap_S2,gamma1f,gamma2f]=OP_cap_N(m1,omega1,m2,omega2,m3,omega3,m4,omega4,m5,omega5,m6,omega6,NO,gammadbCU1,gammadbCU2,gammadbCU3,Psdb,P1db,NOL,alpha1,alpha2)
...
for jj=1:length(Psdb)
    Ps(jj)=10^(Psdb(jj)/10);
    
    gammabars=Ps(jj)/NO;% gamabars is the avg power transmitted at S.
    ...
    for ii=1:NOL
        ...
        gamma1E=(alpha2*gammabars*X4)/(alpha1*gammabars*X4+alpha2*gammabars*X6+1);
        gamma2E=(alpha1*gammabars*X4)/(alpha2*gammabars*X6+1); % SINR of eavesdropper against UE2.
        
        
        
        gamma1rr(ii)=gamma1;
        gamma2rr(ii)=gamma2;
        gamma1Err(ii)=gamma1E;
        gamma2Err(ii)=gamma2E;
        ...
        gamma1f=[gamma1f,mean(gamma1rr)];
        gamma2f=[gamma2f,mean(gamma2rr)];
