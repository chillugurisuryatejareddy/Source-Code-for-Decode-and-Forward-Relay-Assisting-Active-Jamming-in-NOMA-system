...
function [OP1,OP2,OP1E,OP2E,cap_1,cap_2,cap_1E,cap_2E,cap_S1,cap_S2,gamma1f,gamma2f]=OP_cap_A(m1,omega1,m2,omega2,m3,omega3,m4,omega4,m5,omega5,m6,omega6,NO,gammadbCU1,gammadbCU2,gammadbCU3,Psdb,P1db,NOL,alpha1,alpha2)
%m represents fading severity, omega represents channel mean power, NO
%represents noise, Psdb is power from S, P1db is power of jamming signal,
...
for jj=1:length(Psdb)
    ...
    gammabars=Ps(jj)/NO; % gammabars is the avg power transmitted at S.
    ...
    gamma1f(jj)=0; %SINR of user1.
    gamma2f(jj)=0; %SINR of user2.
    gamma1rr=0; 
    gamma2rr=0;
